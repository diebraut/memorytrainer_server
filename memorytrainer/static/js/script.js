    ...
    function restoreLastStableSelection(fallbackId = null) {
        ...
        if (li.getAttribute('data-type') === 'exercise-package') {
            ...
            loadPackageDetails(pkgId)
                .then(data => {
                    if (data) createDetailPanel(data, level + 2);
                })
                .catch(console.error);
            return;
        }
...
// ---- package details (unten) ----
    async function createDetailPanel(packageData, level) {
        ...
        // Draft-Status
        const isDraft = (host.dataset.mode === 'draft') ||
            String(packageData.id || '').startsWith('pkg-new-');
        ...
        // nodeId sicher setzen (Host -> Daten -> aktuelle Unterkategorie)
        ...
        // WICHTIG: Nur Backend-Wert verwenden – kein globaler Fallback!
        const assignedName = (typeof packageData.assignment === 'string'
            ? packageData.assignment.trim()
            : '');
    ...
    <div class="detail-content">
      ...
      <div class="kt-form-grid">
        ...
        ${
            assignedName
                ? `
              <label>Zugeordnetes Übungspaket</label>
              <div class="kt-assign-line">
                <span id="assigned-file" class="kt-output" style="margin-right:12px;">${escapeHtml(assignedName)}</span>
                <button type="button" class="kt-btn" id="btn-unassign">Zuordnung beenden</button>
              </div>
            `
                : `
              <label for="pkg-filelist">Paketzuordnung</label>
              <div class="kt-file-row">
                <select id="pkg-filelist" size="6" class="kt-input" style="width:50%;"></select>
                <div class="kt-file-btns">
                  <button type="button" class="kt-btn" id="btn-assign">Zuordnen</button>
  ...
  `;
        ...
        style.textContent = `
    .kt-file-row { display:flex; gap:12px; align-items:stretch; }
    .kt-file-btns { display:flex; flex-direction:column; justify-content:space-between; }
    .kt-assign-line { display:flex; align-items:center; gap:12px; }
  `;
        ...
        // Draft: Buttons ausblenden + Verwerfen
        if (isDraft) {
        ...
        const assignBtn = panel.querySelector('#btn-assign');
        ...
        async function fetchUploadsForPackage(pkgId) {
            ...
            for (const url of endpoints) {
                try {
                    ...
                    if (!String(e).includes('HTTP 404') && !String(e).includes('HTTP 405')) throw e;
                }
            }
            return [];
        }
        ...
        async function loadUploadList() {
            ...
            const keep = fileListEl.value;
            const files = await fetchUploadsForPackage(host.dataset.packageId);
        ...
        if (assignBtn && fileListEl) {
            assignBtn.addEventListener('click', async (ev) => {
                ev.preventDefault();
                const val = (fileListEl.value || '').trim();
                if (!val) return;
                assignBtn.disabled = true;
                try {
                    const pkgId = parseInt(host.dataset.packageId || '0', 10);
                    const res = await fetch(`${API_BASE}/package/${encodeURIComponent(pkgId)}/assign/`, {
                        method: 'POST',
                        credentials: 'same-origin',
                        headers: {
                    ...
                    if (res.ok) {
                        // immer frische Daten vom Server holen – kein globaler Cache!
        ...
        if (unassignBtn) {
            unassignBtn.addEventListener('click', async (ev) => {
                ev.preventDefault();
                unassignBtn.disabled = true;
                try {
                    const pkgId = parseInt(host.dataset.packageId || '0', 10);
                    const filename = panel.querySelector('#assigned-file')?.textContent?.trim() || '';
                    const res = await fetch(`${API_BASE}/package/${encodeURIComponent(pkgId)}/unassign/`, {
                        method: 'POST',
                        credentials: 'same-origin',
                        headers: {
                            'Accept': 'application/json',
...
// … innerhalb der pkgs.forEach(pkg => { … })
    ...
    function prepareCategoryInsert(direction /* 'before' | 'after' */) {
        ...
        if (!refLi) {
            alert("Keine Referenz-Kategorie gefunden.");
            return;
        }
    ...
    function prepareSubcategoryInsert() {
        ...
        if (!parentLi) {
            alert('Keine Kategorie im Kontext.');
            return;
        }
    ...
    async function handlePackageUpdate() {
        ...
        const changedEl = host.querySelector('#pkg-changed');
        const descEl = host.querySelector('#pkg-desc');
        const fileListEl = host.querySelector('#pkg-filelist');
        ...
        if (newChanged) payload.changed = newChanged;

        // NEU: gewählte Datei mitspeichern (Paketzuordnung)
        if (fileListEl) {
            const sel = (fileListEl.value || '').trim();
            if (sel) {
                payload.assignment = sel;    // nur setzen, wenn wirklich etwas gewählt wurde
            }
        }
        ...
        try {
            ...
            // Datum im UI aktualisieren (falls Server normalized)
            if (changedEl && (data.changed || newChanged)) {
                changedEl.value = data.changed ? formatDateISO(data.changed) : newChanged;
            }

            // Label im Baum aktualisieren
            const li = document.querySelector(`.tree-panel li[data-type="exercise-package"][data-id="${id}"]`);
            if (li) {
            ...
            // Host-Datasets aktualisieren
            host.dataset.packageName = data.title || newTitle;
            host.dataset.packageChanged = data.changed || newChanged;

            // optional: gemerkte Zuordnung ins Dataset schreiben (falls Frontend das nutzt)
            if (typeof data.assignment !== 'undefined') {
                host.dataset.packageAssignment = data.assignment || '';
            }
    ...
    document.addEventListener('click', (e) => {
        ...
        if (scope === 'category') {
            if (action === 'insert-before') {
                prepareCategoryInsert('before');
                return;
            }
            if (action === 'insert-after') {
                prepareCategoryInsert('after');
                return;
            }
            if (action === 'insert-child') {
                prepareSubcategoryInsert();
                return;
            }
            if (action === 'update') {
                const host = getDetailsHost();
